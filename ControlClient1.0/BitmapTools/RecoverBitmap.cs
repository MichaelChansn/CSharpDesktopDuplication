...
namespace ControlClient1._0.BitmapTools
{
    class RecoverBitmap
    {
        ...
        /**根据开始点的坐标，扣取不同的图形块*/
        public static Bitmap recoverScreenBitmap(List<ShortPoint> difPoints, Bitmap globalBtm, Bitmap difBtm ,Size block)
        {
        ...
        public static Bitmap recoverScreenBitmapClone(List<ShortPoint> difPoints, Bitmap globalBtm, Bitmap difBtm, Size block)
        {
            ...
            foreach (ShortPoint difPoint in difPoints)
            {
                ...
                sw.Start();
                Bitmap temp=difBtm.Clone(new Rectangle(startX, startY, width, height),PixelFormat.Format24bppRgb);//10ms????why???
                sw.Stop();
                ...
                temp = null;
                sw.Stop();
                Console.WriteLine("client:draw->" + sw.ElapsedMilliseconds + "ms");
                sw.Reset();
            }
            g.Dispose();
            return toBtm;
